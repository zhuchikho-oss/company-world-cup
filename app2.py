...
# 根據您提供的圖片提取的 32強 (Round of 32) 完整初始賽程
INITIAL_MATCHES = [
...
def evaluate_bet_payout(bet_mode, stake, legs_info):
    if any(leg["status"] == "未開獎" for leg in legs_info): return "未開獎", 0.0
    if bet_mode == "單注":
...
<div class="main-banner"><div class="main-title">🏆 2026 世界盃智能競猜中心</div><div class="sub-title" style="color:#38bdf8;">【官方 32 強賽程與自動派彩版】</div></div>
...
with tabs[1]:
    ...
    def get_match_card_html(m_id, title_name):
        ...
        s = row['status']
        s_h = str(row['score_home']).strip() if s == "已結算" else "-"
        s_a = str(row['score_away']).strip() if s == "已結算" else "-"
        
        badge = f'<span class="status-badge {"settled" if s=="已結算" else "live" if s=="進行中" else "upcoming"}">{s if s!="已結算" else "已完賽"}</span>'
...
if is_admin:
    with tabs[3]:
        ...
        m_mode = st.radio("動作：", ["✏️ 編輯賽事", "➕ 新增賽事"], horizontal=True)
        if m_mode == "✏️ 編輯賽事" and not df_matches.empty:
            sel_m = st.selectbox("賽事", df_matches.apply(lambda r: f"ID:{r['match_id']} | {r['home_team']} VS {r['away_team']}", axis=1))
            ...
            with st.form("e_m"):
                ...
                if st.form_submit_button("💾 儲存"):
                    ...
                    save_sheet(df_matches, "Matches"); st.rerun()
        elif m_mode == "➕ 新增賽事":
            with st.form("a_m"):
                nid = st.text_input("自訂ID (建議32以上)", value="32")
                h, a = st.text_input("主隊"), st.text_input("客隊")
                if st.form_submit_button("➕ 新增"):
                    df_matches = pd.concat([df_matches, pd.DataFrame([{"match_id": nid, "home_team": h, "away_team": a, "status": "未開賽", "score_home": "", "score_away": "", "first_goal_player": ""}])], ignore_index=True)
                    save_sheet(df_matches, "Matches"); st.rerun()
...
if is_admin:
    with tabs[4]:
        ...
        st.info("💡 **一鍵智能派彩**：您不需手動判定注單！只需輸入最終比分，系統將自動校對波膽、讓分、大小球等所有玩法，並根據馬會規則執行「過關容錯派彩計算」，同時將獲勝隊伍自動前推晉級！")
        
        unsettled = df_matches[df_matches["status"] != "已結算"]
        if unsettled.empty: st.markdown("### 🎉 所有賽事皆已結算完畢！")
            ...
            with st.container(border=True):
                ...
                c1, c2, c3 = st.columns(3)
                sh = c1.number_input("🏠 主隊進球", min_value=0, value=0)
                sa = c2.number_input("✈️ 客隊進球", min_value=0, value=0)
                ...
                if st.button("📊 一鍵自動對獎、發送派彩並晉級球隊", type="primary", use_container_width=True):
                    with st.spinner('引擎高速運算中...'):
                        # 1. 更新賽果
                        df_matches.loc[df_matches["match_id"].astype(str) == s_mid, ["status", "score_home", "score_away", "first_goal_player"]] = ["已結算", str(sh), str(sa), fg.strip()]
                        
                        # 2. 自動晉級
                        if int(s_mid) <= 30: calculate_next_stage_advancement(s_mid, win_t)
                            
                        # 3. 自動批改這場比賽的所有注單 (Detail)
                        ...
                        if not df_bets.empty:
                            ...
                            for i, bet in df_bets[df_bets["status"] == "未開獎"].iterrows():
                                ...
                                if not any(legs["status"] == "未開獎"):  # 該單全部賽事都開獎了才結算
                            ...
                            save_sheet(df_users, "Users")
                    st.success(f"🎉 結算成功！已依容錯規則派發獎金！【{win_t}】已自動推向下一輪。")
