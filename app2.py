...
# ==================== 1. 初始化 Excel 資料庫 ====================
DB_FILE = "world_cup_data.xlsx"

def init_db():
    """如果 Excel 檔案不存在，初始化建立包含基礎表結構的 Excel"""
    if not os.path.exists(DB_FILE):
        with pd.ExcelWriter(DB_FILE, engine='openpyxl') as writer:
            # 用戶表 (初始10個同事)
            users = [f"同事 {i}" for i in range(1, 11)]
            ...
            # 賽事表
            df_matches = pd.DataFrame(columns=["match_id", "home_team", "away_team", "status", "score_home", "score_away", "first_goal_player"])
            df_matches.to_excel(writer, sheet_name="Matches", index=False)
            
            # 賠率表
            df_odds = pd.DataFrame(columns=["odd_id", "match_id", "play_type", "selection", "odds_value"])
            df_odds.to_excel(writer, sheet_name="Odds", index=False)
            
            # 投注表
            df_bets = pd.DataFrame(columns=["bet_id", "user_id", "bet_mode", "stake", "status", "win_amount"])
            df_bets.to_excel(writer, sheet_name="Bets", index=False)
            
            # 投注詳情表 (支援過關)
            df_bet_details = pd.DataFrame(columns=["detail_id", "bet_id", "match_id", "odd_id", "selection", "odds_value", "status"])
            df_bet_details.to_excel(writer, sheet_name="BetDetails", index=False)
...
st.markdown("""
    ...
    <div class="sub-header">內部專屬娛樂・資產排名結算系統</div>
...
with tabs[1]:
    ...
    if df_matches[df_matches['status'] == '未開賽'].empty:
            ...
            if match_all_odds.empty:
                ...
                if st.button("📊 確認賽果，一鍵自動派彩！", type="primary", use_container_width=True):
                    with st.spinner('系統結算中，請稍候...'):
                        ...
                        time.sleep(1) # 增加過渡動畫感
                        
                    st.success("🎉 本場賽事結算完成！贏家的虛擬積分已自動派發，排行榜已實時刷新。")
                    time.sleep(1.5)
                    st.rerun()
